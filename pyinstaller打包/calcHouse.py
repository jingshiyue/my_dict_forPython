...
if __name__ == '__main__':
    ...
    if _type == 1:
        ...
        while 1:
            try:
                year_sum = int(input("输入查看前N年累计还款: "))
                print("---------------------------")
                if year_sum*12 > time:
                    rasie("年份输入过大!")
                break
            except:
                print("ERROR: N 须为整数且不大于贷款年限 !")
                print("*********************************")

        print("="*50)
        print("前%d年累计还款如下:" % year_sum)
        ...
        for i in range(1,month_sum+1):
            ...
            liXi_sum += liXi_per
        print("="*80)
        print('【等额本息】 前%d年累计还款: %0.2f，其中还本金: %0.2f，利息: %0.2f' %(year_sum, benJin_sum+liXi_sum, benJin_sum, liXi_sum))
        print("="*80)
    elif _type == 2:
        ...
        for liXi in IR[0:idx]:
            ...
            month_counter += 1
        print("="*80)
        print('【等额本金】 前%d年累计还款: %0.2f，其中还本金: %0.2f，利息: %0.2f' %(year_sum, huanBenJin*idx+liXi_sum, huanBenJin*idx, liXi_sum))
        print("="*80)
